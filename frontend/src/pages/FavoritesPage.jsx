...
import { useFavorites, useRemoveFavorite, useAddToCart } from '../features/profile/useProfile';
      ...
      <div className="container" style={{ position: 'relative' }}>
        ...
        <div className="favorites-list" style={{ position: 'relative', zIndex: 2 }}>
          ...
          {!isLoading && favorites.length === 0 && (
            <p style={{ color: '#3d5690' }}>Ваш список вподобань поки що порожній 😢</p>
          )}

          {!isLoading && favorites.map(item => {
            const clothing = item.clothing;
            if (!clothing) return null;

            // Форматування картинки
            const photoSrc = clothing.photo
              ? (clothing.photo.startsWith("data:") ? clothing.photo : `data:image/jpeg;base64,${clothing.photo}`)
              : null;

            return (
              <div className="favorite-item-row" key={item.id}>
                <Link to={`/product?id=${clothing.id}`} className="fav-item-link">
                  <div
                  ...
                  >
                    {!photoSrc && <span className="fav-photo-icon">📷</span>}
                  </div>
