...
async def add_to_favorites(
    ...
    existing_favorite = await crud_favorite.get_favorite(
        db=db, user_id=current_user.id, clothing_id=data.id_clothing
    )

    if existing_favorite:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Товар вже в обраному"
        )
...
async def delete_from_favorites(
    ...
    deleted = await crud_favorite.delete_favorite(
        db=db, user_id=current_user.id, clothing_id=clothing_id
    )
